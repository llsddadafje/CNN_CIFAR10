...
writer = {
    'Adam_train_loss_3': SummaryWriter("./logs/Adam_train_loss_3"),  # 必须要不同的writer
    'Adam_acc_3': SummaryWriter("./logs/Adam_acc_3"),
    'Adam_recall_3': SummaryWriter("./logs/Adam_recall_3"),
    'Adam_precision_3': SummaryWriter("./logs/Adam_precision_3"),
    'Adam_f1_3': SummaryWriter("./logs/Adam_f1_3")
}
...
# learning_rate = 1e-2  # 第一次
# learning_rate = 1e-1  # 第二次
learning_rate = 1e-3  # 第三次
...
if __name__ == '__main__':
    ...
    for i in range(epoch):
        ...
        writer['Adam_train_loss_3'].add_scalar('value', float(train_for_loss / len(train_data_load)), i)
        ...
        with torch.no_grad():
            ...
            print(now.strftime("%Y-%m-%d %H:%M:%S"))  # 输出时间
        ...
        # 将指标保存到 TensorBoard
        writer['Adam_acc_3'].add_scalar('value', float(accuracy_total / test_data_size), i)
        writer['Adam_precision_3'].add_scalar('value', float(precision), i)
        writer['Adam_recall_3'].add_scalar('value', float(recall), i)
        writer['Adam_f1_3'].add_scalar('value', float(f1), i)
        # torch.save(mynet, f'CIAFR_10_{i + 1}_acc_{accuracy_total / test_data_size}.pth')

    writer['Adam_acc_3'].close()
    writer['Adam_precision_3'].close()
    writer['Adam_recall_3'].close()
    writer['Adam_f1_3'].close()
    writer['Adam_train_loss_3'].close()
