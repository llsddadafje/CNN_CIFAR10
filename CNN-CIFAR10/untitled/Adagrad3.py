...
# writer = {
#     'Adagrad_train_loss': SummaryWriter("./logs/Adagrad_train_loss_"),  # 必须要不同的writer
#     'Adagrad_acc': SummaryWriter("./logs/Adagrad_acc_"),
...
writer = {
    'Adagrad_train_loss_3': SummaryWriter("./logs/Adagrad_train_loss_3"),  # 必须要不同的writer
    'Adagrad_acc_3': SummaryWriter("./logs/Adagrad_acc_3"),
    'Adagrad_recall_3': SummaryWriter("./logs/Adagrad_recall_3"),
    'Adagrad_precision_3': SummaryWriter("./logs/Adagrad_precision_3"),
    'Adagrad_f1_3': SummaryWriter("./logs/Adagrad_f1_3")
}
...
# learning_rate = 1e-3  # 第二次实验
learning_rate = 1e-1  # 第三次实验
...
if __name__ == '__main__':
    for i in range(epoch):
        ...
        writer['Adagrad_train_loss_3'].add_scalar('value', float(train_for_loss / len(train_data_load)), i)
        ...
        # 将指标保存到 TensorBoard
        writer['Adagrad_acc_3'].add_scalar('value', float(accuracy_total / test_data_size), i)
        writer['Adagrad_precision_3'].add_scalar('value', float(precision), i)
        writer['Adagrad_recall_3'].add_scalar('value', float(recall), i)
        writer['Adagrad_f1_3'].add_scalar('value', float(f1), i)

        # torch.save(mynet, f'CIAFR_10_{i + 1}_acc_{accuracy_total / test_data_size}.pth')

    # plot(train_loss, test_loss)
    writer['Adagrad_acc_3'].close()
    writer['Adagrad_precision_3'].close()
    writer['Adagrad_recall_3'].close()
    writer['Adagrad_f1_3'].close()
    writer['Adagrad_train_loss_3'].close()
